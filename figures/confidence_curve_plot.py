...
for i in range(len(test_task1s)):
    ...
    for z in z_thresholds:
        ...
        print(f"Number of voxels with z > {z} : {count}")
    ...
    for (z, count), thresh in zip(sorted(voxel_counts.items()), alphas):
        ax.axvline(x=count, color=base_color, linestyle='--', alpha=thresh, label=f'z={z}')
    ax.set_xscale("log")
